            ...
            <article class="markdown-body js-file "
                ...
                <h1>
                    <a name="user-content-hello-judge_name" class="anchor" href="#hello-judge_name"
                       rel="noreferrer"><span
                            class="octicon octicon-link"></span></a>Hello, {{ Auth::user()->username }}!</h1>
                ...
                <p>To download a plugin, <code>/e download</code> is used (or just <code>/e d</code> or <code>/e
                        dl</code>).
                    The syntax is as follows: <code>/e d [job_name]</code>. All of the parameters can be tab-completed.
                    The
                    <code>job_name</code> parameter is the name of the job on <a href="http://ci.tenjava.com"
                                                                                 rel="noreferrer">Jenkins</a>. This
                    supports
                ...
                <p>After downloading a plugin, a success message will be displayed. If an error is displayed, make sure
                    that
                ...
                <h2>
                    <a name="user-content-removing-plugins" class="anchor" href="#removing-plugins"
                       rel="noreferrer"><span
                            class="octicon octicon-link"></span></a>Removing plugins</h2>

                <p>To remove a plugin, <code>/e remove</code> is used (or just <code>/e r</code> or <code>/e rm</code>).
                    The
                ...
                <p>Please use <code>/stop</code> to restart the server at this point, in order to remove the plugin. You
                    may
                ...
                <p>To edit configurations, <code>/e edit</code> is used (or just <code>/e e</code> or <code>/e ed</code>).
                    The syntax is as follows: <code>/e e [plugin_name] [file_name] (updated_paste_url)</code>. All of
                    the
                    parameters can be tab-completed. <code>plugin_name</code> is the name of the plugin to edit files
                    for.
                    ...
                    (<code>/</code>) to the end of a directory and press tab to get a list of files in that directory.
                </p>

                <p>After getting a file, you may edit it on Hastebin. The resulting Hastebin URL from the edit (which
                    can be
                    obtained by using Ctrl+D to edit, Ctrl+S to save, and Ctrl/Cmd+L and Ctrl/Cmd+C to get the URL), is
                    then
                ...
                <p>To see the log file, run <code>/e logs</code> and click the link in the chat. If you need more
                    content,
                    use <code>/e logs [size]</code>, where <code>size</code> is the number of kilobytes of scrollback
                    you
                    would like to receive. If the size is larger than the log file, you will receive the entire log
                    file.
                </p>

                <h2>
                    <a name="user-content-plugins-that-require-worlds" class="anchor"
                       href="#plugins-that-require-worlds"
