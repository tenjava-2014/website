        ...
        <div class="grid-100">
            ...
            @foreach ($judges as $judge)
                ...
                <div class="judge-progress" style="width: 100%; background-color: #aaa; padding: 0;">
                    <div style="margin: 0; background-color: {{ $judge_progress[$judge->github_name]['finished'] ? "#7DB500" : "#3590E6" }}; width: {{{ $per }}}%; padding: 0;">
                        <h3 style="white-space: nowrap; margin: 0; padding: 5px; color: #fff;">{{{ $judge->github_name }}} ({{{ $judge_progress[$judge->github_name]['assigned'] }}}/{{{ $judge_progress[$judge->github_name]['completed'] }}})</h3>
                    </div>
                </div>
            @endforeach
