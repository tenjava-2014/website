...
class AppController extends BaseController {
    ...
    public function acceptJudgeApp() {
        ...
        $gmail = $app->gmail;
        Mail::queue(array('text' => 'emails.judge.accept'), ['user' => $username], function ($message) use ($gmail) {
...
//        $client->api('repo')->collaborators()->add('tenjava', $username, $username);
    ...
    public function declineJudgeApp() {
        ...
        $username = $app->gh_username;
        $gmail = $app->gmail;
        Mail::queue(array('text' => 'emails.judge.decline'), ['user' => $username], function ($message) use ($gmail) {
            /** @var \Illuminate\Mail\Message $message */
            $message->from('tenjava@tenjava.com', 'ten.java Team');
            $message->to($gmail)->subject('Your recent judge application');
        });
    ...
    public function deleteUserEntry() {
        ...
        if ($te !== null) {
            Queue::push('TenJava\QueueJobs\TimeRemovalJob', ['username' => $app->gh_username, 't1' => $te->t1, 't2' => $te->t2, 't3' => $te->t3]);
            $te->delete();
        }
        $app->delete();
        return Redirect::back();
    ...
    public function listApps($filter = null) {
        ...
        switch ($filter) {
            ...
            case 'search':
                ...
                $viewData['append'] = ['search' => Input::get('search')];
    ...
    public function processApplication($type) {
        ...
        if ($type === 'participant') {
            ...
            $app->save();
            return View::make('pages.result.thanks.participant')->with(['username' => $this->auth->getUsername()]);
        } else {
            ...
            $app->save();
            return View::make('pages.result.thanks.judge')->with(['username' => $this->auth->getUsername()]);
        }
    }
    ...
    public function showApplyJudge() {
        ...
        return View::make('pages.forms.judge', ['username' => $this->auth->getUsername()]);
    }

    public function showApplyParticipant() {
        $this->setActive('sign up');
        $this->setPageTitle('Registration');
        return View::make('pages.forms.participant', ['username' => $this->auth->getUsername()]);
    }
